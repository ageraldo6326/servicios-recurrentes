...
<div class="space-y-5" x-data>
    ...
    @else
        <div class="grid gap-4 xl:grid-cols-[minmax(190px,.7fr)_minmax(190px,.7fr)_minmax(220px,.9fr)_minmax(0,2.2fr)] xl:gap-0 xl:overflow-hidden xl:rounded-2xl xl:border xl:border-line xl:bg-surface xl:shadow-card">
            ...
            <section class="surface min-h-[34rem] overflow-hidden xl:rounded-none xl:border-0 xl:shadow-none">
                @if ($selectedPage)
                    <div wire:key="editor-{{ $selectedPage->id }}-{{ $selectedPage->content_version }}" x-data="notebookEditor({{ $selectedPage->id }}, {{ $selectedPage->content_version }}, @js($selectedPage->title ?? ''), @js($selectedPage->html()))" x-init="init()" x-on:notebook-page-saved.window="saved($event.detail)" x-on:notebook-conflict.window="conflict($event.detail)" x-on:notebook-save-error.window="error($event.detail)" x-on:notebook-editor-focus.window="focus($event.detail.target)" x-on:notebook-editor-reload.window="$wire.selectPage(pageId)" x-on:notebook-image-inserted.window="insertImage($event.detail)" class="flex min-h-[34rem] flex-col">
                        <div class="flex flex-wrap items-center justify-between gap-3 border-b border-line px-4 py-3 sm:px-5"><div class="flex items-center gap-2"><span class="h-2 w-2 rounded-full" :class="status === 'error' || status === 'conflict' ? 'bg-red-500' : (status === 'saving' ? 'animate-pulse bg-amber-500' : 'bg-emerald-500')"></span><span class="text-xs font-bold text-muted" aria-live="polite" x-text="statusText"></span></div><div class="flex gap-2"><button type="button" wire:click="toggleFavorite({{ $selectedPage->id }})" class="button-secondary min-h-9 px-3 text-xs" aria-label="{{ $selectedPage->is_favorite ? 'Quitar de favoritas' : 'Agregar a favoritas' }}">{{ $selectedPage->is_favorite ? '★ Favorita' : '☆ Favorita' }}</button><button type="button" wire:click="deletePage({{ $selectedPage->id }})" class="button-secondary min-h-9 px-3 text-xs text-red-600">Papelera</button></div></div>
                        <div class="flex gap-1 overflow-x-auto border-b border-line bg-surface-soft/60 px-3 py-2" aria-label="Herramientas del editor"><button type="button" class="editor-tool" x-on:click="command('bold')" aria-label="Negrita"><strong>B</strong></button><button type="button" class="editor-tool italic" x-on:click="command('italic')" aria-label="Cursiva">I</button><button type="button" class="editor-tool underline" x-on:click="command('underline')" aria-label="Subrayado">U</button><button type="button" class="editor-tool line-through" x-on:click="command('strikeThrough')" aria-label="Tachado">S</button><span class="mx-1 border-l border-line"></span><button type="button" class="editor-tool" x-on:click="command('formatBlock', 'H1')">H1</button><button type="button" class="editor-tool" x-on:click="command('formatBlock', 'H2')">H2</button><button type="button" class="editor-tool" x-on:click="command('insertUnorderedList')" aria-label="Lista">• Lista</button><button type="button" class="editor-tool" x-on:click="command('insertOrderedList')" aria-label="Lista numerada">1. Lista</button><button type="button" class="editor-tool" x-on:click="checklist()">☐</button><button type="button" class="editor-tool" x-on:click="command('formatBlock', 'blockquote')" aria-label="Cita">❝</button><button type="button" class="editor-tool" x-on:click="link()">Enlace</button><button type="button" class="editor-tool" x-on:click="code()">&lt;/&gt;</button><button type="button" class="editor-tool" x-on:click="imageAlt()">Alt imagen</button><button type="button" class="editor-tool" x-on:click="command('insertHorizontalRule')" aria-label="Separador">—</button></div>
                        <div class="min-h-0 flex-1 overflow-y-auto px-5 py-5 sm:px-8"><input x-ref="title" x-on:input="schedule()" class="w-full border-0 bg-transparent p-0 text-2xl font-black tracking-tight text-ink placeholder:text-muted focus:ring-0 sm:text-3xl" placeholder="Página sin título" aria-label="Título de la página"><div x-ref="body" contenteditable="true" role="textbox" aria-multiline="true" aria-label="Contenido de la página" x-on:input="schedule()" x-on:click="checkboxChanged($event)" class="notebook-editor mt-5 min-h-[20rem] outline-none"></div><p x-show="status === 'conflict'" class="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900 dark:border-amber-900 dark:bg-amber-950/40 dark:text-amber-100">Esta página cambió en otra pestaña. Tu contenido local sigue aquí: cópialo o <button type="button" class="font-bold underline" x-on:click="$wire.selectPage(pageId)">recarga la versión actual</button>.</p><p x-show="status === 'error'" class="mt-4 rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-800 dark:border-red-900 dark:bg-red-950/40 dark:text-red-200"><span x-text="message"></span> <button type="button" class="font-bold underline" x-on:click="save()">Reintentar</button></p></div>
...
@once
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('notebookEditor', (pageId, version, title, html) => ({
                pageId, version, initialTitle: title, initialHtml: html, timer: null, inFlight: false, sentRevision: 0, revision: 0, status: 'saved', message: '', dirty: false,
                get statusText() { return this.status === 'saving' ? 'Guardando…' : this.status === 'error' ? 'Error al guardar' : this.status === 'conflict' ? 'Conflicto de edición' : 'Guardado'; },
                init() { this.$refs.title.value = this.initialTitle; this.$refs.body.innerHTML = this.initialHtml; window.addEventListener('beforeunload', (event) => { if (this.dirty) { event.preventDefault(); event.returnValue = ''; } }); },
                schedule() { this.revision++; this.dirty = true; this.status = 'saving'; clearTimeout(this.timer); this.timer = setTimeout(() => this.save(), 1200); },
                save() { if (this.status === 'conflict') return; if (this.inFlight) { this.timer = setTimeout(() => this.save(), 250); return; } this.inFlight = true; this.sentRevision = this.revision; this.status = 'saving'; this.$wire.saveEditor(this.pageId, this.$refs.title.value, this.$refs.body.innerHTML, this.version); },
                saved(detail) { if (detail.pageId !== this.pageId) return; this.inFlight = false; this.version = detail.version; if (this.revision > this.sentRevision) { this.save(); return; } this.status = 'saved'; this.dirty = false; },
                conflict(detail) { if (detail.pageId !== this.pageId) return; this.inFlight = false; this.status = 'conflict'; this.message = 'Existe una versión más reciente.'; },
                error(detail) { this.inFlight = false; this.status = 'error'; this.message = detail.message; },
                focus(target) { this.$refs[target]?.focus(); },
                command(command, value = null) { this.$refs.body.focus(); document.execCommand(command, false, value); this.schedule(); },
                checklist() { this.command('insertHTML', '<ul><li><input type="checkbox" disabled> Tarea</li></ul>'); },
                code() { this.command('formatBlock', 'pre'); },
                link() { const url = window.prompt('Pega una URL segura (https://, mailto: o tel:)'); if (url) this.command('createLink', url); },
                imageAlt() { const image = this.$refs.body.querySelector('img:last-of-type'); if (!image) return; const alt = window.prompt('Texto alternativo para la última imagen insertada', image.alt || ''); if (alt !== null) { image.alt = alt; this.schedule(); } },
                checkboxChanged(event) { if (event.target?.type === 'checkbox') { event.target.toggleAttribute('checked', event.target.checked); this.schedule(); } },
                insertImage(detail) { const image = document.createElement('img'); image.src = detail.url; image.alt = detail.alt || ''; image.loading = 'lazy'; this.$refs.body.append(image); this.schedule(); },
            }));
        });
    </script>
@endonce
