<div wire:poll.60s>
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div><p class="mb-2 text-xs font-bold uppercase tracking-[0.18em] text-brand">Operación recurrente</p><h1 class="text-2xl font-black tracking-tight text-ink sm:text-4xl">Servicios contratados</h1><p class="mt-2 max-w-2xl text-sm text-muted">Administra los servicios activos, su facturación y proveedor desde un solo lugar.</p></div>
        <a href="{{ route('contracted-services.create') }}" class="button w-full sm:w-auto">＋ Nuevo servicio</a>
    </div>

    <div class="mb-5 grid gap-5 lg:grid-cols-[minmax(0,1.1fr)_minmax(360px,0.9fr)] lg:items-start">
        <div class="panel lg:sticky lg:top-24">
            ...
            <div class="grid gap-3">
                ...
                <button wire:click="clearFilters" class="button-secondary w-full">Limpiar filtros</button>
            </div>
            <div wire:loading class="mt-3 text-xs font-semibold text-brand">Actualizando resultados…</div>
        </div>

        <div class="grid h-full gap-3 lg:grid-rows-4">
            <div class="card flex flex-col justify-center border-brand/20 bg-brand/[0.03] p-4"><p class="text-[11px] font-bold uppercase tracking-[0.16em] text-muted">Ingresos mensuales</p><p class="mt-1 text-xl font-black text-brand">USD {{ number_format($monthlyRevenue, 2) }}</p><p class="mt-1 text-[11px] text-muted">Según filtros activos</p></div>
            <div class="card flex flex-col justify-center p-4"><p class="text-[11px] font-bold uppercase tracking-[0.16em] text-muted">Costos mensuales</p><p class="mt-1 text-xl font-black text-ink">USD {{ number_format($monthlyCosts, 2) }}</p><p class="mt-1 text-[11px] text-muted">Costo de proveedores</p></div>
            <div class="card flex flex-col justify-center p-4"><p class="text-[11px] font-bold uppercase tracking-[0.16em] text-muted">Beneficio mensual</p><p class="mt-1 text-xl font-black {{ $monthlyProfit >= 0 ? 'text-emerald-700 dark:text-emerald-300' : 'text-red-600 dark:text-red-300' }}">USD {{ number_format($monthlyProfit, 2) }}</p><p class="mt-1 text-[11px] text-muted">Ingresos menos costos</p></div>
            <div class="card flex flex-col justify-center p-4"><p class="text-[11px] font-bold uppercase tracking-[0.16em] text-muted">Servicios activos</p><p class="mt-1 text-xl font-black text-ink">{{ $activeCount }}</p><p class="mt-1 text-[11px] text-muted">Coinciden con el filtro</p></div>
        </div>
    </div>
